"""SMTP email delivery (ported from ZhuLinsen范式). Sends the brief as HTML."""
...
def send(subject: str, body_md: str) -> bool:
    ...
    if not (sender and pw):
        log.warning("email not configured; printing instead\n%s", body_md); return False
    receivers = [x.strip() for x in os.getenv("EMAIL_RECEIVERS", sender).split(",") if x.strip()]
    ...
    msg["Subject"] = subject
    msg["From"] = formataddr(("Cockpit 投研助手", sender))
    ...
    try:
        ...
        log.info("email sent to %s", receivers); return True
    except Exception as e:
        log.error("email failed: %s", e); return False
